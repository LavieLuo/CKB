...
class AlexNet_Feature(nn.Module):
    def __init__(self):
        ...
        for i in range(6):
            self.linear.add_module(str(i), alexnet.classifier[i])

    def forward(self, x):
        x = self.features(x)
        x = self.avgpool(x)
        x = x.view(x.size(0), 256 * 6 * 6)
        x = self.linear(x) # 4096d features
        return x
...
class FC_Layers(nn.Module):
    def __init__(self, input_dim, fc_dim_1, fc_dim_2, num_cls):
        ...
        self.fc3 = nn.Linear(fc_dim_2, num_cls) 
